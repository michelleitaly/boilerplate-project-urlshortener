...
app.post(
  ...
  (req, res) => {
    ...
    let urlRegex=new RegExp(/((([A-Za-z]{3,9}:(?:\/\/)?)(?:[-;:&=\+\$,\w]+@)?[A-Za-z0-9.-]+(:[0-9]+)?|(?:www.|[-;:&=\+\$,\w]+@)[A-Za-z0-9.-]+)((?:\/[\+~%\/.\w-_]*)?\??(?:[-\+=&;%@.\w_]*)#?(?:[\w]*))?)/)
    if(!inputUrl.match(urlRegex)){
      console.log("invalid url");
     return  res.json({"error": "Invalid URL"})
    };
    let dataFound = {};
    Url.findOne({ original_url: inputUrl }, (error, data) => {
      if (error) {
        console.log(error);
      } else {
        dataFound = data;
    ...
    Url.findOne({})
      .sort({ short_url: "desc" })
      .exec((error, result) => {
        if (!error && result !== null && dataFound === null) {
          console.log("short + 1");
          inputShort = result.short_url + 1;
        }
        if (!error && dataFound !== null) {
          console.log("dataFound exist and going to print it",  dataFound.original_url, dataFound.short_url);
          res.json({
            original_url: dataFound.original_url,
            short_url: dataFound.short_url,
          });
